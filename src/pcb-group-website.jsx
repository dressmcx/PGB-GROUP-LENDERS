          ...
          <div className="anim-fade-up" style={{ padding: "0 16px 18px", borderTop: `1px solid ${C.ivory}` }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {DEAL_STAGES.map(ds => (
                  <button key={ds.id}
                    onClick={() => { onStageChange(client.id, ds.id); }}
...
function ClientsView({ clients, onAdd, onEdit, onDelete, onStageChange }) {
  ...
  const filtered = clients
    .filter(c => {
      ...
      const match = !q || [c.fullName, c.email, c.phone, c.address, c.notes]
  ...
  return (
    <div style={{ padding: "20px 16px 90px", maxWidth: 800, margin: "0 auto" }}>
      {/* Header row */}
      ...
      <div style={{ background: "white", borderRadius: 14, padding: "16px", marginBottom: 16, border: `1px solid ${C.ivoryDark}` }}>
        <input
          type="text" placeholder="🔍  Search clients by name, email, phone…"
...
// DASHBOARD
// ─────────────────────────────────────────────────────────────
function Dashboard({ lenders, clients, onCategorySelect, onNavigate }) {
  ...
  const totalPipeline = clients.reduce((sum, c) => sum + (c.paymentAmount || 0), 0);
  const paidTotal     = clients.filter(c => c.paymentStatus === "paid").reduce((sum, c) => sum + (c.paymentAmount || 0), 0);
  const pendingTotal  = totalPipeline - paidTotal;
  ...
  return (
    <div style={{ padding: "20px 16px 90px", maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div key={i} className="card-hover" style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <div style={{ fontSize: 22, fontWeight: "bold", fontFamily: "Georgia, serif", color: C.charcoal }}>
                  {kpi.value}
                </div>
              </div>
        ...
        <div style={{ background: "white", borderRadius: 14, padding: "18px 16px", marginBottom: 20, border: `1px solid ${C.ivoryDark}` }}>
          ...
          {recentClients.map(c => {
            ...
            return (
              <div key={c.id} style={{
                display: "flex", alignItems: "center", gap: 12,
                padding: "10px 0", borderBottom: `1px solid ${C.ivory}`,
              }}>
                <div style={{
      ...
      <div className="card-hover" style={{
        ...
        <div onClick={() => setExpanded(v => !v)} style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ color: "#999", fontSize: 11, marginTop: 2 }}>{lender.contactName} · {lender.brokerType} · {lender.location}</div>
          </div>
        ...
        {expanded && (
          <div className="anim-fade-up" style={{ padding: "4px 16px 18px", borderTop: `1px solid ${C.ivory}` }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginTop: 14 }}>
              ...
              <div><div style={fieldLabel}>TYPE</div><div style={{ color: C.charcoal, fontSize: 13 }}>{lender.brokerType}</div></div>
...
// ─────────────────────────────────────────────────────────────
// LENDER LIST / LOCATION / CATEGORY (preserved + styled)
// ─────────────────────────────────────────────────────────────
...
function SearchView({ lenders, onEdit, onDelete }) {
  ...
  const results = lenders.filter(l => {
    const qLow = q.toLowerCase();
    const match = !q || [l.bankName, l.contactName, l.email, l.phone, l.assistantPhone, l.assistantName, l.brokerType, l.notes, String(l.rate)]
      .some(f => (f || "").toLowerCase().includes(qLow));
    ...
    <div style={{ padding: "20px 16px 90px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <div style={{ background: "white", border: `1px solid ${C.ivoryDark}`, borderRadius: 14, padding: "24px 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <SelectField label="Broker Type" value={form.brokerType} onChange={v => set("brokerType", v)} options={BROKER_TYPES} />
...
export default function App() {
  ...
  const activeNav = ["dashboard","clients","search"].includes(view) ? view : "categories";
  ...
  const nextId = client.length > 0 ? Math.max(...clients.map(l => Number(l.id) || 0)) + 1 : 1;
}
  ...
  const renderMain = () => {
    switch (view) {
      ...
      default:
        return <Dashboard lenders={lenders} clients={clients} onCategorySelect={handleCategorySelect} onNavigate={navigate} />;
    }
  };
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "Georgia, serif", background: C.bg, minHeight: "100vh" }}>
        ...
        <main style={{ minHeight: "calc(100vh - 58px)", paddingBottom: 70 }}>
          {renderMain()}
        </main>
