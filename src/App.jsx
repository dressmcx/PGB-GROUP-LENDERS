...
const btnPrimary = {
  ...
  fontFamily: "Georgia, serif", letterSpacing: "3px", cursor: "pointer",
  fontWeight: "bold",
};
const btnSmall = {
...
const labelStyle = {
  color: "#888", fontSize: 10, letterSpacing: 2, marginBottom: 6,
};

// ── "Delete" text-link — replaces all trash icons ─────────────
const deleteLink = {
...
// ── Real PCB Logo ─────────────────────────────────────────────
// Place your logo at public/pcb-logo.png
...
// ── Login ─────────────────────────────────────────────────────
// Fields start BLANK — user must type credentials
function LoginScreen({ onLogin }) {
  const [email,    setEmail]    = useState("");   // ← blank by default
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ ...labelStyle, color: `${C.gold}88`, marginBottom: 6 }}>
            EMAIL ADDRESS
          </div>
          <input
            type="email"
        ...
        {error && (
          <div style={{ color: "#e07070", fontSize: 12, marginTop: 10, letterSpacing: 0.5 }}>
            {error}
          </div>
        )}
        ...
        <div style={{
          ...
          color: C.gold, opacity: 0.35, fontSize: 10, letterSpacing: 2,
        }}>
  ...
  return (
    <div style={{ padding: "32px 24px", maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 32 }}>
        <div style={{ color: C.goldDark, fontSize: 11, letterSpacing: 4, marginBottom: 6 }}>
          OVERVIEW
        </div>
        <h1 style={{
          color: C.charcoal, margin: 0, fontSize: 28,
      ...
      <div style={{
        ...
        {stats.map(s => (
          <div key={s.cat} onClick={() => onCategorySelect(s.cat)} style={{
            ...
            <div style={{ fontSize: 32, fontWeight: "bold", color: C.charcoal, fontFamily: "Georgia, serif" }}>
              {s.count}
            </div>
        ...
        <div style={{
          ...
          padding: "24px 20px", borderLeft: `4px solid ${C.goldDark}`,
        }}>
          <div style={{ color: C.gold, fontSize: 10, letterSpacing: 3, marginBottom: 8 }}>
            TOTAL LENDERS
          </div>
          <div style={{ fontSize: 32, fontWeight: "bold", color: C.ivory, fontFamily: "Georgia, serif" }}>
            {lenders.length}
          </div>
      ...
      {/* Recent lenders */}
      <div style={{ color: C.goldDark, fontSize: 11, letterSpacing: 4, marginBottom: 16 }}>
        RECENT LENDERS
      </div>
      {recentLenders.map(l => (
        <div key={l.id} style={{
          ...
          <div>
            <div style={{ fontWeight: "bold", color: C.charcoal, fontSize: 15 }}>
              {l.bankName}
            </div>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ color: C.goldDark, fontWeight: "bold", fontSize: 18 }}>
              {l.rate}%
            </div>
...
function LocationSelect({ category, lenders, onSelect, onDeleteLocation }) {
  ...
  return (
    <div style={{ padding: "32px 24px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        {locs.map(loc => (
          <div key={loc} style={{
            ...
            {/* "Delete" text link — only for real locations */}
            {loc !== "All Locations" && (
              <button
                onClick={() => setConfirmLoc(loc)}
                style={deleteLink}
                title={`Delete all ${loc} lenders`}
              >
                Delete
              </button>
            )}
      ...
      {/* "Delete All" text link for whole category */}
      <div style={{ marginTop: 28, textAlign: "right" }}>
        <button onClick={() => setConfirmLoc("__ALL__")} style={deleteLink}>
          Delete all lenders in this category
        </button>
      </div>
          ...
          <div style={{
            ...
            {lender.notes && (
              <div style={{
                ...
                <div style={{ color: C.charcoal, fontSize: 13, lineHeight: 1.6 }}>
                  {lender.notes}
                </div>
              </div>
            )}
...
function Field({ label, value, link }) {
  return (
    <div>
      <div style={{ color: "#aaa", fontSize: 10, letterSpacing: 2, marginBottom: 3 }}>
        {label.toUpperCase()}
      </div>
      {link
        ? (
...
function LenderList({ lenders, category, location, onEdit, onDelete, onDeleteAll, onAdd, onBack }) {
  ...
  return (
    <div style={{ padding: "32px 24px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <h2 style={{
            ...
            {location}{" "}
            <span style={{ color: "#aaa", fontSize: 16 }}>({filtered.length})</span>
          </h2>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
          {filtered.length > 0 && (
            <button onClick={() => setConfirmAll(true)} style={deleteLink}>
              Delete all
            </button>
          )}
          <button onClick={onAdd} style={{ ...btnPrimary, width: "auto", marginTop: 0, padding: "10px 22px" }}>
            + ADD LENDER
          </button>
        </div>
      </div>
      ...
      {confirmAll && (
        <DeleteModal
          message={`Delete all ${filtered.length} lender(s) shown here? This cannot be undone.`}
    ...
    <div style={{ padding: "32px 24px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...inputStyle,
            color: C.charcoal, background: C.bg,
            border: `1px solid ${C.ivoryDark}`, marginBottom: 12,
          }}
        />
    ...
    <div style={{ padding: "32px 24px", maxWidth: 700, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div style={{ gridColumn: "1/-1" }}>
            ...
            <textarea value={form.notes} onChange={e => set("notes", e.target.value)} rows={3}
              style={{
                ...inputStyle, color: C.charcoal, background: C.bg,
                border: `1px solid ${C.ivoryDark}`,
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 24, alignItems: "center" }}>
          <button onClick={() => onSave(form)} style={{ ...btnPrimary, width: "auto", marginTop: 0, padding: "12px 28px" }}>
            {initial ? "SAVE CHANGES" : "ADD LENDER"}
          </button>
          <button onClick={onCancel} style={{ ...btnSmall, padding: "12px 20px" }}>
            CANCEL
          </button>
        </div>
      </div>
...
function Sidebar({ activeNav, onNavigate }) {
  ...
  return (
    <nav style={{
      ...
      borderRight: `1px solid ${C.goldDark}22`,
  ...
  const renderMain = () => {
    switch (view) {
      ...
      case "lenders":
        return (
          <LenderList
            lenders={lenders} category={selectedCategory} location={selectedLocation}
      ...
      case "location":
        return (
          <LocationSelect
            category={selectedCategory} lenders={lenders}
