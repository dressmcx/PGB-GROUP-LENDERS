    ...
    <svg width={size} height={size * 0.55} viewBox="0 0 220 120" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="goldGrad" x1="0" y1="0" x2="1" y2="1">
          ...
          <stop offset="50%" stopColor="#C7B89A" />
          <stop offset="100%" stopColor="#A89878" />
        </linearGradient>
      </defs>
      {/* P */}
      <text x="4" y="80" fontFamily="Georgia, serif" fontSize="78" fontWeight="bold" fill="url(#goldGrad)">P</text>
      {/* Pyramid triangle inside C circle space */}
      <circle cx="108" cy="45" r="34" fill="none" stroke="url(#goldGrad)" strokeWidth="6"/>
      <polygon points="108,22 90,65 126,65" fill="url(#goldGrad)" opacity="0.85"/>
      {/* B */}
      <text x="140" y="80" fontFamily="Georgia, serif" fontSize="78" fontWeight="bold" fill="url(#goldGrad)">B</text>
      {/* GROUP subtitle */}
      <text x="6" y="108" fontFamily="Georgia, serif" fontSize="16" letterSpacing="6" fill="#C7B89A" opacity="0.8">GROUP</text>
    </svg>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <PCBLogo size={200} />
        <div style={{ color: C.gold, letterSpacing: "8px", fontSize: 13, fontFamily: "Georgia, serif", marginTop: 8 }}>
          PYRAMID CAPITAL BROKERAGE
        </div>
        <div style={{ width: 60, height: 1, background: C.gold, opacity: 0.5, marginTop: 8 }} />
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width: "100%", maxWidth: 420, boxShadow: "0 0 60px rgba(0,0,0,0.6)",
        border: `1px solid ${C.goldDark}33`,
      }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <PCBLogo size={160} />
          <div style={{ color: C.gold, letterSpacing: "5px", fontSize: 11, marginTop: 12 }}>
        ...
        />
        {error && <div style={{ color: "#e07070", fontSize: 12, marginTop: 8, letterSpacing: 0.5 }}>{error}</div>}
...
function Header({ user, onLogout, onHome }) {
  return (
    <header style={{
      ...
      <div style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }} onClick={onHome}>
        <PCBLogo size={100} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 32 }}>
        {stats.map(s => (
          <div key={s.cat} onClick={() => onCategorySelect(s.cat)} style={{
            ...
            onMouseEnter={e => { e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.1)"; e.currentTarget.style.borderColor = C.gold; }}
            onMouseLeave={e => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = C.ivoryDark; }}
          >
        ...
        <div style={{
          background: C.charcoal, borderRadius: 2, padding: "24px 20px",
          borderLeft: `4px solid ${C.goldDark}`,
        }}>
          ...
          <div style={{ fontSize: 12, color: C.gold, opacity: 0.6, marginTop: 4 }}>
            Avg Rate: {(lenders.reduce((a, l) => a + l.rate, 0) / lenders.length).toFixed(2)}%
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 20 }}>
        {CATEGORIES.map((cat, i) => (
          <div key={cat} onClick={() => onSelect(cat)} style={{
            background: "white", border: `1px solid ${C.ivoryDark}`,
            borderRadius: 2, padding: "32px 24px", cursor: "pointer",
            textAlign: "center", transition: "all 0.2s",
          }}
            onMouseEnter={e => { e.currentTarget.style.background = C.charcoal; e.currentTarget.querySelector(".cat-title").style.color = C.ivory; e.currentTarget.querySelector(".cat-desc").style.color = C.gold; }}
            onMouseLeave={e => { e.currentTarget.style.background = "white"; e.currentTarget.querySelector(".cat-title").style.color = C.charcoal; e.currentTarget.querySelector(".cat-desc").style.color = "#888"; }}
          >
            <div style={{ fontSize: 36, marginBottom: 12 }}>{icons[i]}</div>
            <div className="cat-title" style={{ fontFamily: "Georgia, serif", fontSize: 16, fontWeight: "bold", color: C.charcoal, transition: "color 0.2s" }}>{cat}</div>
            <div className="cat-desc" style={{ fontSize: 11, color: "#888", marginTop: 8, lineHeight: 1.5, transition: "color 0.2s" }}>{descs[i]}</div>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16 }}>
        {locs.map(loc => (
          <div key={loc} onClick={() => onSelect(loc)} style={{
            background: "white", border: `1px solid ${C.ivoryDark}`,
            borderLeft: `4px solid ${C.gold}`,
            borderRadius: 2, padding: "20px 16px", cursor: "pointer", transition: "all 0.2s",
          }}
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#888", marginTop: 6 }}>{counts(loc)} lender{counts(loc) !== 1 ? "s" : ""}</div>
          </div>
        ))}
      </div>
    </div>
...
function Breadcrumb({ items, onNavigate }) {
  return (
    <div style={{ color: "#aaa", fontSize: 11, letterSpacing: 1, marginBottom: 24 }}>
      <span style={{ cursor: "pointer", color: C.goldDark }} onClick={() => onNavigate && onNavigate(-1)}>Dashboard</span>
      {items.map((item, i) => (
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <span style={{ color: C.gold, fontSize: 18, transition: "transform 0.2s", display: "inline-block", transform: expanded ? "rotate(180deg)" : "rotate(0deg)" }}>▾</span>
        </div>
      </div>
      ...
      {expanded && (
        <div style={{ padding: "0 20px 20px", borderTop: `1px solid ${C.ivory}` }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginTop: 16 }}>
            ...
            {lender.assistantPhone && <Field label={`Assistant (${lender.assistantName || "Asst."})`} value={lender.assistantPhone} link={`tel:${lender.assistantPhone}`} />}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            ...
            <button onClick={(e) => { e.stopPropagation(); onDelete(lender.id); }} style={{ ...btnSmall, background: "#fee", color: "#c44", borderColor: "#fcc" }}>DELETE</button>
          </div>
        </div>
      )}
    </div>
...
function Field({ label, value, link }) {
  return (
    <div>
      ...
      {link ? <a href={link} style={{ color: C.charcoal, fontSize: 13, textDecoration: "none", borderBottom: `1px solid ${C.gold}` }}>{value}</a>
...
// ── Lender List ───────────────────────────────────────────────
function LenderList({ lenders, allLenders, category, location, onEdit, onDelete, onAdd, onBack }) {
...
function SearchView({ lenders, onEdit, onDelete }) {
  ...
  const results = lenders.filter(l => {
    ...
    const match = !q || [l.bankName, l.contactName, l.email, l.phone, l.assistantPhone,
      l.assistantName, l.brokerType, l.notes, String(l.rate)].some(f => (f || "").toLowerCase().includes(qLow));
      ...
      <div style={{ background: "white", border: `1px solid ${C.ivoryDark}`, borderRadius: 2, padding: "28px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div>
            ...
            <select value={form.category} onChange={e => set("category", e.target.value)} style={{ ...selectStyle, width: "100%", boxSizing: "border-box" }}>
              {CATEGORIES.map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
          <div>
            <div style={labelStyle}>Broker Type</div>
            <select value={form.brokerType} onChange={e => set("brokerType", e.target.value)} style={{ ...selectStyle, width: "100%", boxSizing: "border-box" }}>
    ...
    <nav style={{
      ...
      {links.map(l => (
        <div key={l.id} onClick={() => onNavigate(l.id)} style={{
          ...
          background: view === l.id ? `${C.goldDark}22` : "transparent",
          borderLeft: view === l.id ? `3px solid ${C.gold}` : "3px solid transparent",
          color: view === l.id ? C.gold : "#888",
          transition: "all 0.15s", fontSize: 13, letterSpacing: 1,
        }}
          onMouseEnter={e => { if (view !== l.id) e.currentTarget.style.color = C.ivory; }}
          onMouseLeave={e => { if (view !== l.id) e.currentTarget.style.color = "#888"; }}
        >
...
export default function App() {
  ...
  const [view, setView] = useState("dashboard"); // dashboard | categories | location | lenders | form | search
  ...
  const handleSave = (form) => {
    ...
    if (formMode === "edit") {
      setLenders(ls => ls.map(l => l.id === editingLender.id
        ? { ...form, id: editingLender.id, rate: parseFloat(form.rate) || 0, minLoan: parseInt(form.minLoan) || 0, maxLoan: parseInt(form.maxLoan) || 0 }
        : l
      ));
      setView("lenders");
    } else {
      const newLender = { ...form, id: Date.now(), rate: parseFloat(form.rate) || 0, minLoan: parseInt(form.minLoan) || 0, maxLoan: parseInt(form.maxLoan) || 0 };
  ...
  const renderMain = () => {
    ...
    if (view === "location") return (
      <LocationSelect category={selectedCategory} lenders={lenders} onSelect={handleLocationSelect} />
    );
    if (view === "categories") return (
      <CategorySelect onSelect={handleCategorySelect} />
    );
    return <Dashboard lenders={lenders} onCategorySelect={(cat) => { setSelectedCategory(cat); setView("location"); }} />;
  };
  ...
  return (
    <div style={{ fontFamily: "Georgia, serif", background: C.bg, minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex" }}>
        <Sidebar view={["dashboard", "categories", "location", "lenders", "form"].includes(view) ? (view === "dashboard" ? "dashboard" : view === "search" ? "search" : "categories") : view} onNavigate={navigate} />
